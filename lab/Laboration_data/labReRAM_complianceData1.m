...
%{
Task 1: Current Compliance data 1
...
%% Extracting compliance current data1 - RESET
...
[compResetData1] = xlsread(['Compliance_Data_1','\','RESET_01.csv'],...
                 ['B259:C2500']);  % max voltage = -2
compReset1_V_TE = compResetData1(:,1);
compReset1_I_TE = abs(compResetData1(:,2));
n1 = size(compResetData1,1);
R1 = zeros(n1,1);
for (i=1:n1)
    R1(i) = abs(compReset1_V_TE(i))/compReset1_I_TE(i);
end
% Low Resistive States (LRS)
LRS1 = mean(R1);
%[maxRes1,ind1] = max(abs(R1));
    
% the point where the resistance is 30 % of the resistance at max voltage
[maxV1, ind1] = max(abs(compReset1_V_TE));
maxI1 = compReset1_I_TE(ind1);
maxR1 = maxV1/maxI1;

switchR1 = 0.3*maxR1;
switchI1 = 0.3*maxI1;
compI = 100e-6;
%resetV1 = compI*switchR1 % 9.39
resetV1 = switchI1*switchR1;
%switchReset1 = -0.3*max(abs(compReset1_V_TE))

%%
[compResetData2] = xlsread(['Compliance_Data_1','\','RESET_02.csv'],...
                 ['B259:C2500']);  % max voltage = -2
compReset2_V_TE = compResetData2(:,1);
compReset2_I_TE = abs(compResetData2(:,2));
n2 = size(compResetData2,1);
R2 = zeros(n2,1);
for (i=1:n2)
    R2(i) = abs(compReset2_V_TE(i))/compReset2_I_TE(i);
end
% Low Resistive States (LRS)
LRS2 = mean(R2);
%[maxRes1,ind1] = max(abs(R1));
...
% the point where the resistance is 30 % of the resistance at max voltage
[maxV2, ind2] = max(abs(compReset2_V_TE));
maxI2 = compReset2_I_TE(ind2);
maxR2 = maxV2/maxI2;
...
xlabel('Voltage (V)')
ylabel('Current (A)')
title('Compliance current 1, I = 100 uA')
